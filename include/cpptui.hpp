...
struct TextHelper {
  ...
  static std::vector<CharInfo>
  prepare_text_for_render(const std::string &text) {
...
inline bool
handle_scrollbar_event(const Event &event, int x, int y, int width, int height,
                       int content_size, int &scroll_offset, bool &is_dragging,
                       bool horizontal,
...
public:
  ...
  static int clamp_size(int size, int min_val, int max_val) {
    if (max_val > 0 && size > max_val)
      size = max_val;
    if (min_val > 0 && size < min_val)
      size = min_val; // min wins over max
    return size;
  }
...
public:
  ...
  std::string get_selected_text() const {
    ...
    return TextHelper::utf8_substr(text_, selection_state_.start,
...
public:
  ...
  wrap_text_static(const std::string &input, int max_width, int first_indent,
                   int hang_indent, bool preserve_ws = false) {
...
template <typename State> class UndoRedoHistory {
public:
...
public:
  ...
  std::string get_selected_text() const {
    ...
    return TextHelper::utf8_substr(value_, selection_state_.start,
                                   selection_state_.end -
                                       selection_state_.start);
  }
...
protected:
  ...
  static void
  distribute_flex(const std::vector<std::shared_ptr<Widget>> &children,
                  int available, bool use_height, std::vector<int> &out_sizes) {
        ...
        if (event.x >= x && event.x < x + width && event.y >= y &&
          ...
          if (event.ctrl) {
            ...
            if (event.mouse_wheel_up()) { // Wheel Up -> Scroll Left
              scroll_x -= step;
              handled = true;
            } else if (event.mouse_wheel_down()) { // Wheel Down -> Scroll Right
              scroll_x += step;
              handled = true;
            }
          } else {
    ...
    if (event.is_key_event() && has_focus()) {
      ...
      if (event.ctrl) {
        if (event.is_nav_home()) { // Ctrl+Home -> Leftmost
          ...
          scroll_x -= vw;
          handled = true;
        } else if (event.is_view_scroll_down()) { // Ctrl+PageDown -> Right Page
          scroll_x += vw;
          handled = true;
        }
      } else {
...
public:
  ...
  Tabs() {
    ...
    next_btn_ = std::make_shared<Button>(next_marker, [this]() {
      if (scroll_offset_ < (int)tab_names.size() - 1)
        scroll_offset_++;
    });
  }
